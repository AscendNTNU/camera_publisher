...
int main(int argc, char** argv){
  ...
  bool publish_compressed_jpeg = false;
  ...
  unsigned char* compressed_buffer = new unsigned char[out_w*out_h*3];
  ...
  while(ros::ok()){
    webcam_capture_image(input_buffer, &compressed_buffer, &compressed_buffer_size);
    if(w == out_w && h == out_h){
    ...
    if(publish_compressed_jpeg) {
        ...
        compressed_output_image.format = "jpeg";
